...
/*   Updated: 2024/10/22 15:27:44 by mohamibr         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
		...
		t_env_cpy *env)
{
	...
	if (**input == '?')
	{
		...
		var_value = ft_itoa(env->last_exit_status); 
			// Get the last exit status as a string
...
static int	handle_redirection_token(char **input, char **token,
		t_token **token_list, t_env_cpy *env, int *error_flag, char *quote_type)
{
...
static void	handle_quotes_and_expansion(char **input, char **token,
		...
		int *error_flag)
{
	...
	while (**input && **input != ' ' && *error_flag == 0)
	{
		if ((**input == '<' || **input == '>'))
		{
			if (handle_redirection_token(input, &token, token_list, env,
					error_flag, &quote_type))
				return ;
			break ;
		}
		else if (**input == '"')
